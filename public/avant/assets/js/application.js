...
$(function () {
    ...
    $.each ($('ul.acc-menu a'), function() {
        ...
        strLocationModules = strLocation.substr(strLocation.indexOf('index.php') + 10);

        arrLocation = strLocationModules.split('/');
        locationModule = arrLocation[0];

		strHref = this.href;
        strHrefModules = strHref.substr(strHref.indexOf('index.php') + 10);
        //alert(strHrefLocation);

	   	arrHref = strHrefModules.split('/');
        hrefModule = arrHref[0];

		//alert(hrefModule + '--' + locationModule);
		
        if( hrefModule == locationModule ) {
            if(arrLocation.length > 1){ //Nếu đang ở action của module, check nếu là index_anotherindex và không
                if((arrLocation[1].indexOf('index') != -1) && (arrLocation[1] != 'index') ){
                    if((arrHref.length > 1) && (arrLocation[1] == arrHref[1]) ){
                        targetAnchor = this;
                        return false;
                    }
                }else{
